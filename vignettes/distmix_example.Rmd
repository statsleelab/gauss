...
In this vignette, we will show how to use distmix() to impute association Z-scores of unmeasureds SNPs in multi-ethic cohorts.  
...
The distmix() function requires four key arguments:

1. Path to the association Z-score data file.
2. Path to the mixing proportion data file.
3. Path to the reference panel index file (i.e., reference_index_file).
4. Path to the reference data file (i.e., reference_data_file).
5. Path to the reference population description file (i.e., reference_pop_desc_file).

The association Z-score data file should include six columns with the following column names: rsid (SNP ID), chr (chromosome number), bp (base pair position), a1 (reference allele), a2 (alternative allele), and z (association Z-score). The mixing proportion data file should be a text file including two columns with the following names: pop (populatin abbreviation) wgt (weight or proportion).
